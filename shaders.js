...
fn hit_quad(qd: Quad, ro: vec3f, rd: vec3f, tmin: f32, tmax: f32, h: ptr<function, Hit>) -> bool {
  ...
  (*h) = Hit(t, p, select(-nn, nn, front), front, mat, vec2f(0.0), 0i);
  return true;
}
...
fn hit_sphere_s(s: Sphere, ro: vec3f, rd: vec3f, tmin: f32, tmax: f32, h: ptr<function, Hit>) -> bool {
  ...
  (*h) = Hit(t, p, select(-n_out, n_out, front), front, mat, vec2f(0.0), 0i);
  return true;
}
...
@group(0) @binding(33) var atlas3 : texture_2d<f32>;
@group(0) @binding(34) var atlas4 : texture_2d<f32>;
@group(0) @binding(35) var atlas5 : texture_2d<f32>;
...
fn atlas_sample(page: u32, uv: vec2f) -> vec3f {
  if (page == 0u) { return textureSampleLevel(atlas0, texSamp, uv, 0.0).rgb; }
  if (page == 1u) { return textureSampleLevel(atlas1, texSamp, uv, 0.0).rgb; }
  if (page == 2u) { return textureSampleLevel(atlas2, texSamp, uv, 0.0).rgb; }
  if (page == 3u) { return textureSampleLevel(atlas3, texSamp, uv, 0.0).rgb; }
  if (page == 4u) { return textureSampleLevel(atlas4, texSamp, uv, 0.0).rgb; }
  if (page == 5u) { return textureSampleLevel(atlas5, texSamp, uv, 0.0).rgb; }
  if (page == 6u) { return textureSampleLevel(atlas6, texSamp, uv, 0.0).rgb; }
  if (page == 7u) { return textureSampleLevel(atlas7, texSamp, uv, 0.0).rgb; }
  return vec3f(0.0);
}
...
fn sample_albedo(h: Hit) -> vec3f {
  return tex_color(h.mat.tex, h.p, h.mat.alb);
}
`;
...
fn trace(ro0: vec3f, rd0: vec3f, rng: ptr<function, u32>) -> vec3f {
  ...
  loop {
    ...
    tp = tp * sample_albedo(h) * (scat / pdf);
    ro = h.p + h.n * 0.002;
    rd = chosen;
  }
